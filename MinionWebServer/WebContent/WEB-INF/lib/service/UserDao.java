...
public class UserDao {
	...
	public boolean registerUserDevice(String userID, String deviceID) {
		...
		logger.info("Registrando el id de dispositivo para el usuario = " + userID);
		...
		try {
			...
			logger.info("Número de Registros actualizados: " + i);
	...
	public void getUserByID(String userID) {
		...
		logger.info("Obteniendo usuario a partir del USU_ID = " + userID);
		...
		try {
			...
			if (rs.next()) {
				...
				logger.warn("No se ha encontrado ningún usuario con el USU_ID = " + userID);
			}
		} catch (SQLException e) {
			logger.error("Ha ocurrido un error" + e.getMessage(), e);
		} finally {
	...
	private void asignarAtributos(ResultSet rs) throws SQLException {
		...
		logger.info("Asignando atributos para el usuario USU_ID = " + usuID);
